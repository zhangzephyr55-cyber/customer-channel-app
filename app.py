...
def get_effective_customers(salesperson=None):
    conn = get_db()
    yr = str(CURRENT_DATE.year)+"-01-01"
    ls = str(CURRENT_DATE.year-1)+"-01-01"
    le = str(CURRENT_DATE.year-1)+"-12-31"
    ya = str(ONE_YEAR_AGO)
    if salesperson:
        rows = conn.execute("""
            ...
            FROM transactions t WHERE t.salesperson=?
            GROUP BY t.customer_code HAVING order_count_last_year>=3 ORDER BY t.customer_name
        """, [ya, ls, le, yr, salesperson]).fetchall()
        ...
        """).fetchall()
    result = []
    for row in list(rows)+list(new_rows):
        d = dict(row)
        if salesperson:
...
def api_customers():
    ...
    return jsonify({"customers": get_effective_customers(name), "salesperson": name})
...
def api_save():
    ...
    for ch in channels:
        conn.execute("""
            INSERT INTO channel_fillings (customer_code, salesperson, supermarket, wholesale, cs, ecommerce, outlet, snack_discount, instant_retail, other, notes, updated_at)
...
def admin_upload():
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row[hmap["客户代码"]] is None: continue
            txns.append({
        ...
        new_cust = conn.execute("SELECT COUNT(*) FROM new_customers").fetchone()[0]
        eff = conn.execute("""
            SELECT COUNT(*) FROM (
        ...
        """, [str(ONE_YEAR_AGO)]).fetchone()[0] + new_cust
        filled = conn.execute("""
            SELECT COUNT(*) FROM (
                SELECT DISTINCT f.customer_code FROM channel_fillings f
                WHERE ROUND(f.supermarket+f.wholesale+f.cs+f.ecommerce+f.outlet+f.snack_discount+f.instant_retail+f.other,1)=100
            )
        """).fetchone()[0]
        conn.close()
        return jsonify({"success":True, "message":f"上传成功！共导入 {len(txns)} 条记录",
...
def admin_import():
    ...
    try:
        ...
        if "渠道备份" not in wb.sheetnames:
            return jsonify({"error":"缺少 渠道备份 工作表"}), 400
        ws = wb["渠道备份"]
        hdrs = [cell.value for cell in ws[1]]
        kmap = {}
        for h in ["客户代码","业务员"]+CHANNEL_KEYS:
            for i, ch in enumerate(hdrs):
                if ch and str(ch).strip()==h:
                    kmap[h]=i; break
        missing = [h for h in ["客户代码","业务员"]+CHANNEL_KEYS if h not in kmap]
        if missing:
            return jsonify({"error":f"格式不匹配，缺少: {', '.join(missing)}"}), 400
        conn = get_db()
        restored=0
        for row in ws.iter_rows(min_row=2, values_only=True):
            cc = str(row[kmap["客户代码"]] or "").strip()
            sp = str(row[kmap["业务员"]] or "").strip()
            if not cc or not sp: continue
            vals = {k: float(row[kmap[k]] or 0) for k in CHANNEL_KEYS}
            conn.execute("""
                INSERT INTO channel_fillings (customer_code,salesperson,supermarket,wholesale,cs,ecommerce,outlet,snack_discount,instant_retail,other,notes,updated_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,datetime('now','localtime'))
                ON CONFLICT(customer_code,salesperson) DO UPDATE SET
...
def admin_export():
    ...
    for ri, cust in enumerate(customers, 2):
        ...
        for ji, k in enumerate(CHANNEL_KEYS):
            ...
            if k == "ecommerce" and v > 0: has_ec = True
        ws.cell(row=ri, column=8+len(CHANNEL_NAMES), value="是" if has_ec else "否")
        ws.cell(row=ri, column=8+len(CHANNEL_NAMES)+1, value=cust.get("notes",""))
    ...
    bk = ["客户代码","客户","业务员","大区","省份"]+CHANNEL_KEYS
    for ci, h in enumerate(bk, 1):
        ws2.cell(row=1, column=ci, value=h).font = openpyxl.styles.Font(bold=True)
    conn = get_db()
    fill_rows = conn.execute("""
        ...
        LEFT JOIN (SELECT DISTINCT customer_code,customer_name,region,province FROM transactions) t ON f.customer_code=t.customer_code
...
def admin_stats():
    ...
    new_cust = conn.execute("SELECT COUNT(*) FROM new_customers").fetchone()[0]
    eff = conn.execute("""
        SELECT COUNT(*) FROM (SELECT customer_code FROM transactions
        GROUP BY customer_code HAVING COUNT(DISTINCT CASE WHEN date>=? THEN doc_no ELSE NULL END)>=3)
    """, [str(ONE_YEAR_AGO)]).fetchone()[0] + new_cust
    filled = conn.execute("""
        SELECT COUNT(*) FROM (SELECT DISTINCT customer_code FROM channel_fillings
        WHERE ROUND(supermarket+wholesale+cs+ecommerce+outlet+snack_discount+instant_retail+other,1)=100)
    """).fetchone()[0]
    sp_rows = conn.execute("SELECT DISTINCT salesperson FROM (SELECT salesperson FROM transactions UNION SELECT salesperson FROM new_customers) ORDER BY salesperson").fetchall()
    pstats = []
    for sp in sp_rows:
        sp_name = sp["salesperson"]
        sp_new = conn.execute("SELECT COUNT(*) FROM new_customers WHERE salesperson=?", [sp_name]).fetchone()[0]
        total = conn.execute("""
            SELECT COUNT(*) FROM (SELECT customer_code FROM transactions WHERE salesperson=?
            GROUP BY customer_code HAVING COUNT(DISTINCT CASE WHEN date>=? THEN doc_no ELSE NULL END)>=3)
        """, [sp_name, str(ONE_YEAR_AGO)]).fetchone()[0] + sp_new
...
def admin_analysis_data():
    customers = get_effective_customers()
    total = len(customers)
    filled_c = [c for c in customers if abs(sum(float(c.get(k,0)) for k in CHANNEL_KEYS)-100)<0.1]
    filled = len(filled_c)
    ch_data = {}
    for k,n in zip(CHANNEL_KEYS, CHANNEL_NAMES):
        vals = [float(c.get(k,0)) for c in filled_c]
        ch_data[n] = {"total":round(sum(vals),1),"avg":round(sum(vals)/len(vals),1) if vals else 0,
                      "count_gt0":sum(1 for v in vals if v>0),
                      "pct_of_customers":round(sum(1 for v in vals if v>0)/total*100,1) if total else 0}
    dominant = {}
    for c in filled_c:
        mx = max((float(c.get(k,0)),k) for k in CHANNEL_KEYS)
        if mx[0]>0:
            n = CHANNEL_NAMES[CHANNEL_KEYS.index(mx[1])]
            dominant[n] = dominant.get(n,0)+1
    regions = {}
    for c in customers:
        r = c.get("region","未知") or "未知"
        if r not in regions: regions[r]={"total":0,"filled":0}
        regions[r]["total"]+=1
        if abs(sum(float(c.get(k,0)) for k in CHANNEL_KEYS)-100)<0.1:
            regions[r]["filled"]+=1
    has_ec = sum(1 for c in filled_c if float(c.get("ecommerce",0))>0)
    return jsonify({"total_customers":total,"filled_customers":filled,
                    "completion_pct":round(filled/total*100,1) if total else 0,
                    "channels":ch_data,"dominant_channel":dominant,
                    "regions":regions,"has_ecommerce":has_ec})
...
def admin_analysis_export_ppt():
    ...
    bg = sl.background.fill; bg.solid(); bg.fore_color.rgb = RGBColor(0,47,167)
    txBox = sl.shapes.add_textbox(Inches(1), Inches(2.5), Inches(11), Inches(2))
    tf = txBox.text_frame; tf.word_wrap = True
    p = tf.paragraphs[0]; p.text="客户渠道数据分析报告"; p.font.size=Pt(36); p.font.color.rgb=RGBColor(255,255,255); p.font.bold=True
    p2=tf.add_paragraph(); p2.text=f"报告日期：{CURRENT_DATE.year}年{CURRENT_DATE.month}月{CURRENT_DATE.day}日"; p2.font.size=Pt(18); p2.font.color.rgb=RGBColor(200,200,255)
    p3=tf.add_paragraph(); p3.text=f"客户总数：{len(customers)} 家 | 已填写：{len(filled_c)} 家"; p3.font.size=Pt(16); p3.font.color.rgb=RGBColor(200,200,255)
    sl2=prs.slides.add_slide(prs.slide_layouts[6])
    t2=sl2.shapes.add_textbox(Inches(0.5),Inches(0.3),Inches(12),Inches(0.8)); t2.text_frame.paragraphs[0].text="渠道分布概览"; t2.text_frame.paragraphs[0].font.size=Pt(24); t2.text_frame.paragraphs[0].font.bold=True
    table=sl2.shapes.add_table(10,5,Inches(0.5),Inches(1.2),Inches(10),Inches(5)).table
    for ci,h in enumerate(["渠道","总占比","平均占比","覆盖客户数","覆盖率(%)"]):
        cell=table.cell(0,ci); cell.text=h
        for pp in cell.text_frame.paragraphs: pp.font.size=Pt(11); pp.font.bold=True; pp.font.color.rgb=RGBColor(255,255,255)
        cell.fill.solid(); cell.fill.fore_color.rgb=RGBColor(0,47,167)
    for ri,(k,n) in enumerate(zip(CHANNEL_KEYS,CHANNEL_NAMES)):
        vals=[float(c.get(k,0)) for c in filled_c]; avg=round(sum(vals)/len(vals),1) if vals else 0
        tv=round(sum(vals),1); cnt=sum(1 for v in vals if v>0); pct=round(cnt/len(customers)*100,1) if customers else 0
        table.cell(ri+1,0).text=n; table.cell(ri+1,1).text=str(tv); table.cell(ri+1,2).text=str(avg); table.cell(ri+1,3).text=str(cnt); table.cell(ri+1,4).text=str(pct)
        for ci in range(5):
            cell=table.cell(ri+1,ci)
            for pp in cell.text_frame.paragraphs: pp.font.size=Pt(10)
            if ri%2==0: cell.fill.solid(); cell.fill.fore_color.rgb=RGBColor(240,242,250)
    table.cell(9,0).text="合计"
    for pp in table.cell(9,0).text_frame.paragraphs: pp.font.bold=True
    table.cell(9,1).text=str(round(sum(float(c.get(k,0)) for c in filled_c for k in CHANNEL_KEYS),1))
    for pp in table.cell(9,1).text_frame.paragraphs: pp.font.bold=True
    sl3=prs.slides.add_slide(prs.slide_layouts[6])
    t3=sl3.shapes.add_textbox(Inches(0.5),Inches(0.3),Inches(12),Inches(0.8)); t3.text_frame.paragraphs[0].text="关键洞察"; t3.text_frame.paragraphs[0].font.size=Pt(24); t3.text_frame.paragraphs[0].font.bold=True
    ins=[f"客户总数：{len(customers)} 家",f"已填写渠道：{len(filled_c)} 家（{round(len(filled_c)/len(customers)*100,1) if customers else 0}%）"]
    ec=sum(1 for c in filled_c if float(c.get("ecommerce",0))>0)
    ins.append(f"有电商业务：{ec} 家")
    top=""; md=0; dom={}
    for c in filled_c:
        mx=max((float(c.get(k,0)),k) for k in CHANNEL_KEYS)
        if mx[0]>0:
            n=CHANNEL_NAMES[CHANNEL_KEYS.index(mx[1])]; dom[n]=dom.get(n,0)+1
            if dom[n]>md: md=dom[n]; top=n
    if top: ins.append(f"主力渠道：{top}（{md} 家）")
    tb=sl3.shapes.add_textbox(Inches(0.5),Inches(1.3),Inches(10),Inches(5))
    tf3=tb.text_frame; tf3.word_wrap=True
    for i,ins_text in enumerate(ins):
        pp=tf3.paragraphs[0] if i==0 else tf3.add_paragraph()
        pp.text=f"• {ins_text}"; pp.font.size=Pt(16); pp.space_after=Pt(12)
